...
def validate_config(config: dict, *, require_coordinates: bool = True) -> dict:
    ...
    if config.keys() - {"schema_version", "solver", "removals", "urban", "images", "demands", "maps"}:
    ...
    for map_config in maps:
        ...
        if map_config.keys() - {"id", "locality", "points", "routes", "margin_m", "max_snap_distance_m",
                                "center", "radius_m", "graphml", "graph_sha256", "removal_edges"}:
        ...
        for route in routes:
            if not isinstance(route, dict) or route.keys() - {"id", "origin", "destination", "demands"}:
...
def map_extent(map_config: dict) -> tuple[tuple[float, float], int]:
    ...
    coordinates = [(p["latitude"], p["longitude"]) for p in map_config["points"].values()]
